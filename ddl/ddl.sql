...
ALTER TABLE round ADD CONSTRAINT FK_round_3
    FOREIGN KEY (fk_instancia_pokemon_id_1???, fk_instancia_pokemon_id_2???, fk_instancia_pokemon_id_2???)
    REFERENCES instancia_pokemon (???, ???, ???);
 
ALTER TABLE round ADD CONSTRAINT FK_round_4
    FOREIGN KEY (???)
    REFERENCES instancia_pokemon (???);
 
ALTER TABLE turno ADD CONSTRAINT FK_turno_1
    FOREIGN KEY (fk_round_id, habilidade_pokemon_1???, habilidade_pokemon_2???)
    REFERENCES round (id, ???, ???)
    ON DELETE RESTRICT;
 
ALTER TABLE turno ADD CONSTRAINT FK_turno_2
    FOREIGN KEY (???)
    REFERENCES habilidade (???);
 
ALTER TABLE turno ADD CONSTRAINT FK_turno_3
    FOREIGN KEY (???)
    REFERENCES habilidade (???);
